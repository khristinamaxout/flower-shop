...
Владелец магазина правит каталог, цены, фотографии и условия доставки
через панель по адресу `/admin.html`. Данные хранятся в Supabase, изменения
видны посетителям сразу — пересобирать сайт не нужно.

Настройка описана в **[ADMIN.md](ADMIN.md)** — там же инструкция для заказчика.

Как устроено:

- `supabase/schema.sql` — таблицы, хранилище фотографий и права доступа
- `src/data/supabase-config.js` — ключи проекта Supabase
- `src/data/public-api.js` — чтение каталога сайтом (обычный `fetch`, без SDK)
- `src/data/api.js` — запись из админки (SDK: вход по паролю, загрузка файлов)
- `src/admin/` — интерфейс панели

Пока ключи не заполнены, сайт работает на встроенном каталоге из
`src/data/products.js`, а панель показывает инструкцию по подключению.
